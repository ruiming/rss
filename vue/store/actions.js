import Vue from 'vue'
import { Feed, Feeds, User, Post, Posts } from '../resource/resource'
...
export const unsubscribe = ({ commit, state }) => Feed.delete({
  id: state.feed.feed._id,
}).then(() => {
...
export const getFeed = ({ commit }, id) => Feed.get({
  id,
}).then(res => {
...
export const getPopularFeeds = ({ commit }, page) => Feeds.popular({
  page: page || 0,
}).then(res => {
...
export const getPost = ({ commit }, id) => Post.get({
  id,
}).then(res => {
...
export const love = ({ commit, state }, post) => Post.update({
  id: (post && post._id) || state.post.post._id,
}, {
  type:   'love',
  revert: true,
...
export const getRecentPosts = ({ commit }) => Posts.recent().then(res => {
  commit(types.RECEIVE_RECENT_POSTS, res.data)
})

export const getFeedPosts = ({ commit, state }, { id, page, per_page }) => Posts.get({
  feed_id:  id,
  page,
...
export const getPosts = ({ commit, state }, type) => Posts.get({
  type,
}).then(res => {
